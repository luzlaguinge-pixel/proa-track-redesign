...
export type AlertTipo = 'perdido' | 'reparacion' | 'linea' | 'baja_pendiente' | 'confirmacion_vencida';
...
const getDashboardStats = (country: Country): DashboardStats => {
  ...
  const materials = country === 'todos' ? all : all.filter(m => m.pais === country);
  ...
  const sinMovimiento90 = materials.filter(m => {
    ...
    const days = (today.getTime() - new Date(m.fechaActualizacion).getTime()) / 86_400_000;
    return days > 90;
  }).length;
  ...
  for (const m of materials) {
    ...
    if (m.estado === 'perdida') {
      alertas.push({ id: m.id, label, sublabel, motivo: 'Perdido', tipo: 'perdido', linkTo: `/inventory/${m.id}` });
    } else if (m.estado === 'en_reparacion') {
      alertas.push({ id: m.id, label, sublabel, motivo: 'En reparación', tipo: 'reparacion', linkTo: `/inventory/${m.id}` });
    } else if ((m.tipo === 'celular' || m.tipo === 'tablet') && m.estado === 'sin_uso' && m.lineaTelefonica !== null) {
      alertas.push({ id: m.id, label, sublabel: m.lineaTelefonica ?? undefined, motivo: 'Línea sin responsable', tipo: 'linea', linkTo: `/inventory/${m.id}` });
    }
  }
  ...
  return { total, enUso, utilizacionPct, disponible, sinMovimiento90, perdidos, enReparacion, alertas };
};
